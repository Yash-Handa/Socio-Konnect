...
const debug = require('debug')('SocioKonnect:confirmationEmail');
...
module.exports = (email, jwt, username) => {
  ...
  <div style="padding: 20px 0 50px; font-family: Lucida Sans Unicode, Lucida Grande, sans-serif;padding-bottom: 40px">
    ...
    <div style="background: #74428D; position: relative; height: 130px; width: 100%; margin-bottom: 30px;">
      <div style="width: 100%; height: 100px; position: relative;">
        <h1 style="margin: 0px auto; font-size: 7vmin; font-weight: 300; color: #eee; line-height: 100px; text-align: center; font-family: Comic Sans MS, cursive, sans-serif"><strong>${config.sender}</strong></h1>
      </div>
      <div style="position: relative; background: #472C66; height: 40px;"><h1 style="margin-left: 20px; margin-top: 0; font-size: 3.5vmin; font-weight: 300; color: #cfd8dc; line-height: 40px;">Confirmation</h1></div>
    </div>
    <div style="width: 80%; margin: auto;margin-bottom: 20px;font-size: 14px;text-align: center;">
      <p style="font-size: 17px; margin: 15px 0 35px">Hey <strong>${username}</strong>, you're almost ready to start enjoying ${config.sender}. Simply click the big Purple Button below to verify your Account :)</p>
      <a href="${config.host}/auth/confirmation/${jwt}" style= "border: 0; text-align: center;background: #74428D;color: #eee;padding: 10px;font-size: 16px;font-weight: 300;max-width: 200px;margin: 20px auto;display: block;cursor: pointer;border-radius: 2px;box-shadow: 0px 5px 6px rgba(0,0,0,0.3);text-decoration: none;">Confirm Account</a>
    ...
    <div style="width: 100%;height: 50px;background: #472C66;position: absolute;bottom: 0;margin:0;">
